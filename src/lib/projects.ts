...
// ââ Metadados sem arquivos (para o Ã­ndice geral) ââââââââââââââââââââââââââââââ
...
// ââ Helpers de tamanho âââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function calcSize(files: Record<string, string>): number {
  return Object.values(files).reduce((a, v) => a + v.length, 0);
}

// ââ Salvar arquivos de um projeto (chave separada) âââââââââââââââââââââââââââ
function saveProjectFiles(id: string, files: Record<string, string>): void {
  ...
  try {
    ...
    try {
      ...
      // Se ainda falhar, tenta salvar sÃ³ metadados (arquivos vazios)
      try {
        const empty: Record<string, string> = {};
        for (const k of Object.keys(files)) empty[k] = "";
        localStorage.setItem(key, JSON.stringify(empty));
      } catch { /* sem espaÃ§o mesmo */ }
    }
  }
}

// ââ Carregar arquivos de um projeto ââââââââââââââââââââââââââââââââââââââââââ
...
// ââ Ãndice de metadados âââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function loadIndex(): ProjectMeta[] {
  try {
    const raw = localStorage.getItem(INDEX_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    // Suporta formato antigo (com files embutido) e novo (sÃ³ metadados)
...
function saveIndex(metas: ProjectMeta[]): void {
  try {
    ...
    // MantÃ©m apenas os 10 mais recentes se falhar
    try {
      const slim = metas
        .slice()
 ...
 * Migra formato antigo automaticamente (arquivos embutidos no Ã­ndice).
 */
  ...
  try {
    ...
    return parsed.map(p => {
      ...
      if (p.files && Object.keys(p.files).length > 0) {
        // Formato antigo: arquivos embutidos no Ã­ndice â migra para novo formato
        files = p.files;
...
/**
 * Salva todos os projetos â arquivos separados do Ã­ndice.
 */
export function saveProjects(projects: Project[]): void {
  ...
  // Salva apenas metadados no Ã­ndice geral
