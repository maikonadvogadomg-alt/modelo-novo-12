...
// âââ Detecta pacote faltando ââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// âââ Voice hook âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// âââ Comandos rÃ¡pidos âââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
const QUICK = [
...
// âââ Formata duraÃ§Ã£o ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
export default function StreamTerminal({
  ...
  // Executa comando externo (IA / botÃµes)
  useEffect(() => {
  ...
  return (
    <div className="h-full flex flex-col bg-[#141c0d] overflow-hidden font-mono text-xs">
      ...
      <div className="flex items-center gap-2 px-3 py-1 bg-[#1c2714] border-b border-gray-700/30 shrink-0 min-h-[28px]">
        ...
        <span className="text-[11px] text-gray-500">{isRunning ? "executandoâ¦" : "pronto"}</span>
        ...
        <div className="shrink-0 bg-[#111a0a] border-b border-green-700/30 px-2 py-2 flex flex-col gap-1.5">
          ...
          <div className="relative">
            <input autoFocus type="text" value={pkgQuery} onChange={e => onPkgQuery(e.target.value)}
              placeholder={pkgMode === "npm" ? "axios, lodash, react-queryâ¦" : "requests, numpy, flaskâ¦"}
      ...
      <div ref={outputRef} className="flex-1 overflow-auto p-2 space-y-3">
        {entries.length === 0 && (
          <div className="text-gray-600 text-[11px] space-y-1 pt-1">
            <p className="text-green-700">ð Pasta de trabalho: ~/sk-projetos</p>
            <p>Terminal pronto. Digite um comando ou clique nos botÃµes acima.</p>
            <p className="text-[10px] opacity-60">â â histÃ³rico Â· Ctrl+C cancelar Â· Ctrl+L limpar</p>
          </div>
        )}
          ...
          return (
            <div key={entry.id} className="space-y-1">
              ...
              <div className="flex items-center gap-1.5 text-[11px] flex-wrap">
                ...
                <button onClick={() => copyOutput(entry)} className="shrink-0 text-gray-600 hover:text-gray-300 transition-colors" title="Copiar saÃ­da">
                ...
                {entry.running ? (
                  ...
                  <span className={`text-[9px] px-1.5 py-0.5 rounded-full ${exitOk ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}>
                    {exitOk ? "â OK" : `â exit ${entry.exitCode}`}
                  </span>
                )}
              ...
              {!entry.running && entry.missingPackage && (
                entry.missingPackage === "__deps__" ? (
                  <div className="flex items-center gap-2 ml-3 p-2 rounded bg-yellow-400/10 border border-yellow-400/30">
                    ...
                    <span className="text-[11px] text-yellow-300 flex-1">DependÃªncias nÃ£o instaladas.</span>
                  ...
                  <div className="flex items-center gap-2 ml-3 p-2 rounded bg-yellow-400/10 border border-yellow-400/30">
                    <Download className="w-3.5 h-3.5 text-yellow-400 shrink-0" />
                    <span className="text-[11px] text-yellow-300 flex-1">Pacote <code className="font-bold">{entry.missingPackage}</code> nÃ£o encontrado.</span>
      ...
      <div className="shrink-0 border-t border-gray-700/30 bg-[#0f1a09] flex items-center px-3 py-2.5 gap-2">
        ...
        <input
          ...
          placeholder={isRunning ? "aguardeâ¦ (Ctrl+C para cancelar)" : "npm install, git status, node server.jsâ¦"}
        ...
        <button
          ...
          title={listening ? "Parar gravaÃ§Ã£o" : "Falar comando"}
        >
