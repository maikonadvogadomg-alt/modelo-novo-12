...
export default function DriveBackupPanel({ onClose, files, projectName }: DriveBackupPanelProps) {
  ...
  const handleBackup = async () => {
    setUploading(true);
    setStatus("ð¦ Compactando projeto...");
    setError("");
    try {
      ...
      setStatus("âï¸ Enviando para o Google Drive...");
      ...
      setStatus(`â Backup "${name}" salvo no Drive!`);
      await loadFiles();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 pt-5 pb-4 border-b border-gray-700/40">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-gray-500 truncate">
              {fileCount} arquivo{fileCount !== 1 ? "s" : ""} Â· ~{sizekb} KB no projeto atual
            </p>
          </div>
        ...
        {/* Ãrea de aÃ§Ã£o â enviar backup */}
        <div className="px-5 py-4 border-b border-gray-700/30">
          ...
          {error && (
            <p className="mt-2.5 text-[12px] text-center text-red-400">â {error}</p>
          )}
        </div>
                ...
                <div key={f.id} className="flex items-center gap-3 p-3 rounded-2xl border border-gray-700/40 bg-[#1c2714] hover:border-gray-600/40 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-gray-600">
                      ...
                      {f.size ? ` Â· ${Math.round(Number(f.size) / 1024)} KB` : ""}
                    </p>
                  </div>
